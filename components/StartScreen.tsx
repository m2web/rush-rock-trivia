...
interface StartScreenProps {
  onStart: () => void;
  error?: string | null;
  /** Milliseconds remaining in the rate-limit cooldown (0 = ready to play) */
...
const StartScreen: React.FC<StartScreenProps> = ({ onStart, error, cooldownRemaining = 0 }) => {
      ...
      {isRateLimited ? (
        <div className="mb-2">
          <p className="text-gray-400 text-sm mb-3">
            You can take another quiz in
          </p>
          <p className="text-red-400 font-mono text-3xl font-bold mb-3 tabular-nums">
            {formatCountdown(cooldownRemaining)}
          </p>
          <button
            disabled
            className="bg-gray-600 text-gray-400 font-bold py-4 px-10 rounded-full text-xl cursor-not-allowed opacity-60"
          >
            Begin the Test
          </button>
        </div>
