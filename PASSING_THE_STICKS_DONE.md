...
- [x] `narrator_voice.mp3` - AI-generated voice-over file (Google Cloud TTS
  recommended)

### 3. Fonts

Will use Google Fonts - "Orbitron" for headings and "Open Sans" for body text
via web links and CSS.
...
```python
# Python code (see docs for details)
# Assumes 'client' is initialized (e.g. from openai import OpenAI; client = OpenAI())
...
- [ ] Scene 2 - Kit transition overlay (crossfade animation) - use
  kit_transition_overlay.png
...
- [ ] Add `index.html` and `passingthesticks.css` using this structure like this
  (I say like as you will need to fill in the details):
...
- [ ] Verify all images resolve correctly under
  `/public/image/passingthesticks/`
- [ ] Verify all audio files resolve correctly under
  `/public/audio/passingthesticks/`
...
- [ ] Once all items are checked off, `/passingthesticks` will be a
  self-contained,
